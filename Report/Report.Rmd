...
The aim of this report is to explain how a KD-Tree is implemented in a serial or parallel way.

In the serial part, the algorithm is constructed as a "normal" program, without keeping in consideration how the processor will works. In the parallel part, it will be consider how the processor can distribuite the amout of work among processes or threads. To implement this part, it was used two famous parallel programming library, that are **OpenMP** and **Open MPI**. 
...
Since that can be assume that the data are homogeneously distributed in every dimension, it was decided to choose $p$ as the median element along each dimension. In this way, the binary three will be balanced^[A tree is balanced if and only if all of the left and the right subtrees' heights differ by at most one].
...
# Implementation
The KD-Tree implementation was made with C++11 and is written to work with the number of k-dimension > 0 (*N_DIM > 0*). The code is a generic programming algorithm, because it's written with C++ templates. 
...
First of all, it was created the performance model of OpenMP and Open MPI. 
The model of the OpenMP part was:
$$ T_{OMP} = \frac{T_{serial}(N)}{NP} + k $$
Where $N$ is the total number of points, $T_{serial}$ is the total time to create a KD-Tree of $N$ points with one process, $NP$ is the number of used processes and $k$ is a constant variable referred to the time to load in run-time some OpenMP start-up stuffs. 

Instead, the model of Open MPI was different, because it has to calculate also the time to serialize/deserialize. In fact, the model is this one:
$$ T_{MPI} = \frac{T_{serial}(N)}{NP - 1} + T_{serialize}(\frac{N-NP-1}{NP}) + T_{deserialize}(\frac{N -NP - 1}{NP}) $$
The time to serialize and deserialize, depends of how much points there are. In fact, the number of points it isn't always $N$, but they are less. This because, the first part of the code is runned by all of the processes and they build the same tree in a "serial" way, until they rich a good level to start to work alone. So, there are $N-NP-1$ because one part of the KD-Tree is already been built. 
Another thing is that are used only $NP-1$ processes for the parallelization, this because the process 0 only have to recieve the KD-Tree, deserialize them and combine them. 

If we want to understand much better the time to serialize/deserialize, it has been made another model:
$$ T_{MPI} = \frac{T_{serial}(N)}{NP - 1} + log(\frac{N-NP-1}{NP}) + NP * k * \frac{N-1}{2}*log(n) $$


<!-- ```{r echo=FALSE, message=FALSE, warning=FALSE,fig.height=4} -->
<!-- #Plot OPENMP-->

<!-- plot_model <- merge(strong_all, model_mp, by="Process") -->

<!-- ggplot(data = plot_model)+ -->
...
<!--     geom_line(mapping = aes(Process, model_mp, col = "Model MP")) + -->
<!--     geom_point(mapping = aes(Process, model_mp, col = "Model MP"))  -->
...
The strong scalability explain the performance of the parallel algorithm  with the number of processes that increase while the problem size remains constant. The individual workload must be kept high enough to keep all processors fully occupied. The speedup achieved by increasing the number of processes usually decreases more or less continuously.
...
Moreover, the serial execution time of an OpenMP program on one thread it's slower than the equivalent serial program because the compiler may not be able to as aggressively optimize the parallel code as the serial code. In this way, at run-time the code may pay a penalty in start-up performance of instantiating the OpenMP run-time stuff even though the code only uses one thread. The run-time doesn't know this in advance.
...
Hence the deserialization part has been processed by the same process (process 0), if there are more total process, the time of the deserializetion will be less because the it will deserialize less points.
